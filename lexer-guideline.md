...
The Lexer operates on the source code as a character stream and transforms it into a stream of Tokens. It must strictly adhere to the following rules:

- **Maximal Munch (Longest Match):** At any given source position, the Lexer must always consume the longest sequence of characters that matches a valid token pattern. *(Example: Input `>=` must be tokenized as a single `GE` operator, not `>` followed by `=`).*
...
- **Error Recovery:** The Lexer **must never crash** on invalid input. Instead, it emits a specific `INVALID` token, records the offending character's exact location, advances exactly one character past the error, and resumes scanning.

---

## 2. Complete Token Category Specifications (Regex Table)
The table below defines the exact Regular Expressions and recognition rules for every Token Category.
...
| **String Literals** | `"hello"`, `"hello\n"` | Quoted strings. Supports escape sequences (`\n`, `\t`, `\"`, `\\`).<br>Regex: `\"([^"\\]|\\.)*\"` |
...
| **Preprocessor Directives** | `#include`, `#define` | Matches entire lines starting with `#` (excluding comments). Emitted as a special `DIRECTIVE` token which the Parser will ignore.<br>Regex: `#.*` |
...
## 3. Lexical Error Handling (Mandatory Rules)
The Lexer must implement the following specific error detection and reporting mechanisms:
...
- **`Token peekToken()`**: Returns the next `Token` object *without* advancing the input stream. This allows the Parser to perform 1-token Lookahead (essential for LL(1) decision making).
